...
const App = () => {
  ...
  const handleDrugDisplay = (e) => {
    if(!e.target.checked) {
      filterTypes = filterTypes.filter(type => !(type === e.target.value))
    }
    setData(drug.filter(drugItem => filterTypes.includes(drugItem.type)))
  };
  return <div>
    <h1>Highlighting and Tagging Demo</h1>
    <div
      ref={refParagraph}
      className='content'
    >
      <TextAnnotator
        ...
        marker={({text, data}) => (
          <mark id={text.replace(/[^a-zA-Z0-9]/g, '')} style={{backgroundColor: colorByType[data.type]}}>{text}</mark>)}
      />
    </div>
    <Popover selectionRef={refParagraph}>
      <PopoverBoxWithButtons/>
    </Popover>
    <TypeList types={types} drug={drugList} colorByType={colorByType} onCheck={handleDrugDisplay}/>
  </div>
}
